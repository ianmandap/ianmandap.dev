...
draft: false
summary: 'A post-incident review of using Universally Unique Identifiers (UUIDs) as a primary key and its implicit ordering'
---
...
by their memory layout<sup>[1]</sup>.

## Implicit Ordering by Rails

In our project, the `Foo` resource used the 
...
It turns out, based on [a Github Issue on the above method](https://github.com/rails/rails/issues/44621), retriving all records of a resource (e.g. `Foo.all`) will
...
chain a `#reorder` method.

As another precaution, we added a compound index on the table based on `created_at` and `id`. This sorts by `created_at` first 
and then by `id` so that records are ordered more predictably (i.e. when records have the same `created_at` they are then sorted by `id`).

#### References
- <sup>[1]</sup> https://stackoverflow.com/a/73822906
- https://github.com/rails/rails/issues/44621
- https://stackoverflow.com/a/30825837
